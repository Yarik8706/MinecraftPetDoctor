...
{
    public class PlayerBehaviour : MonoBehaviour, IAnimalCarrying, IItemCarrying
    {
        ...
        public static Vector3 Position { get => playerBehaviour.transform.position; set => playerBehaviour.transform.position = value; }
        public Transform Transform { get => playerBehaviour.transform; }

        [SerializeField] NavMeshAgent agent;
        [SerializeField] private Material playerMaterial; 
        [SerializeField] private GameObject _regularPlayerGraphic;
        ...
        public void Initialise()
        {
            ...
            SetGraphics(_regularPlayerGraphic, StoreController.GetSelectedSkin(TabType.Character));
        ...
        private void OnProductSelected(TabType tab, ProductData product)
        {
            ...
            {
                ChangeSkin(product.Skin);
            }
        }

        public void ChangeSkin(Texture newSkin)
        {
            playerMaterial.SetTexture("_MainTex", newSkin);
        }

        public void SetGraphics(GameObject graphics, Texture texture)
        {
            // Check if graphics isn't exist already
            if(playerPrefabGraphics != graphics)
            {
                playerMaterial.SetTexture("_MainTex", texture);
                // Store prefab link
                ...
                leftFootTransform = playerGraphics.leftFootTransform;
                rightFootTransform = playerGraphics.rightFootTransform;
        ...
        public static PlayerBehaviour GetBehavior()
        {
        ...
        {
            if (playerBehaviour.isItemsCarrying)
                playerBehaviour.RemoveItems(true);

            if (playerBehaviour.isAnimalCarrying)
                playerBehaviour.DropAnimals(true);
        }
        ...
        public static void SpawnMaxText()
        {
            ...
            FloatingTextController.SpawnFloatingText("Max", 
                new MultiText("МАКС", "MAX").GetText(), playerBehaviour.transform.position + new Vector3(0, 9, -1), Quaternion.Euler(45, 0, 0), 1.0f);
        }
